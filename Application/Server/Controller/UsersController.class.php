...
//    }
    ...
    public function edittime() {
        if (IS_POST) {
            ...
            $result = M('shopmessage')->where(['shopid' => $shopid])->field('gotime')->save($data);
            if ($result) {
                $this->ajaxReturn(['status' => 1, 'msg' => ""]);
            } else {
                $this->ajaxReturn(['status' => 0, 'msg' => ""]);
    ...
    public function shopmesa() {
        if (IS_POST) {
            ...
            $result = M('shopmessage')->where(['shopid' => $shopid])->field("content,gotime,long,maxprice,maxlong,lkmoney,shophead,shopphoto,shopname,state,coordinate,detailed_address,mobile")->find();
            if ($result) {
                //查询当天明细
                $map = array();
